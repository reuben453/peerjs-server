...
app._initializeWSS = function(server) {
  //console.log("_initializeWSS: "+JSON.stringify(this));
  ...
  this._wss.on('connection', util.requireAuth('null', false), function(socket) {
    var query = url.parse(socket.upgradeReq.url, true).query;
    //console.log("socket: ");
    //for (var propName in socket) {
    //  console.log(propName + ": " + socket[propName]);
    //}
...
app._initializeHTTP = function() {
  ...
  this.post('/:key/:id/:token/id', /*_prepareBody,*/ /*passport.authenticate('local', {failureRedirect: '/'}) util.requireAuth('/peerjs', false),*/ function(req, res, next) {
    console.log('http streaming post');
    ...
    req.on('end', function() {
      ...
      //console.log(passport.authenticate('local', id, password)());
    });*/
  ...
  this.get('/register', util.requireAuth('/register', true), function(req, res) {
      res.render('register', { });
  });
  ...
  this.post('/login', passport.authenticate('local'), function(req, res, next) {
